...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  const job = jobs.find((j) => j.title.split(" ").join("-") === params.slug);
  ...
  return {
    ...
    alternates: { canonical: `https://sarkariresult.rest/job/${params.slug}` },
    openGraph: {
      title: job.title,
      description: job.description,
      url: `https://sarkariresult.rest/job/${params.slug}`,
      type: "article",
      images: [
        {
    ...
    <main className="lg:col-span-2 space-y-10">
      ...
      <div className="rounded-xl border bg-white dark:bg-card shadow p-6 space-y-3">
        ...
        <table className="table-auto w-full text-sm border-collapse border border-gray-200">
          <tbody>
            <tr className="border-b">
              <td className="px-4 py-2 font-medium">Qualifications</td>
              <td className="px-4 py-2">{job.eligibility ?? "Refer to official notification."}</td>
            </tr>
            <tr>
              <td className="px-4 py-2 font-medium">Age</td>
              <td className="px-4 py-2">{job.age ?? "Check notification"}</td>
            </tr>
          </tbody>
        </table>
      </div>
            ...
            <tbody>
              {Object.entries(job.importantDates).map(([key, value]) => (
                <tr key={key} className="border-b">
                  <td className="px-4 py-2 capitalize font-medium">{key.replace(/([A-Z])/g, " $1")}</td>
